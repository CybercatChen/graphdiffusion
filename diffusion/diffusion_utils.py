...
def cosine_beta_schedule_discrete(timesteps, nu_arr, s=0.008):
    ...
    nu_arr = np.array(nu_arr)  # X, charges, E, y, pos
...
def linear_schedule(timesteps, nu_arr):
    """
    Adds noise to the input in a linear manner.
    :param timesteps: int. Total number of timesteps to apply noise for
    :param nu_arr: Ignored
    :return: Tensor. Equally spaced time intervals
    """
...
def sample_discrete_features(probX, probE, prob_charges, node_mask, is_directed=False):
    ''' Sample features from multinomial distribution with given probabilities (probX, probE, proby)
        ...
        :param is_directed: Whether the graph is directed or not. Default False
    '''
...
def posterior_distributions(clean_data, noisy_data, Qt, Qsb, Qtb, is_directed):
    ...
    return PlaceHolder(X=prob_X, E=prob_E, charges=prob_c, y=None, pos=None, directed=is_directed)


def sample_feature_noise(z_t):
    """Standard normal noise for all features.
        Output size: X.size(), E.size(), y.size() """
    epsX = torch.randn(z_t.X.size())
    epsE = torch.randn(z_t.E.size())
    epsy = torch.randn(z_t.y.size())

    float_mask = z_t.node_mask.float()
    ...
    return PlaceHolder(X=epsX, E=epsE, y=epsy, charges=z_t.charges, pos=z_t.pos,
                       node_mask=z_t.node_mask, directed=z_t.directed).mask()


def sample_normal(mu_X, mu_E, mu_y, sigma, z_t):
    """
    Takes the parameters and returns a Placeholder object.
    :param mu_X: mu of the nodes
    :param mu_E: mu of edges
    :param mu_y: mu of labels
    :param sigma: sigma for sampling
    :param z_t: Original placeholder object having information about the pos, charge, directed etc
    :return: utils.Placeholder object
    """
    feature_placeholder = PlaceHolder(X=mu_X, E=mu_E, y=mu_y, charges=z_t.charges, t_int=z_t.t_int,
                                      directed=z_t.directed,
                                      t=z_t.t, pos=z_t.pos, node_mask=z_t.node_mask)
    eps = sample_feature_noise(feature_placeholder)
    X = mu_X + sigma * eps.X
    E = mu_E + sigma.unsqueeze(1) * eps.E
    y = mu_y + sigma.squeeze(1) * eps.y
    return PlaceHolder(X=X, E=E, y=y, charges=z_t.charges, t_int=z_t.t_int, directed=z_t.directed,
                       t=z_t.t, pos=z_t.pos, node_mask=z_t.node_mask)
