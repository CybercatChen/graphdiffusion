...
class ExtraFeatures:
    def __init__(self, extra_features_type, dataset_info):
        ...
        if extra_features_type in ['eigenvalues', 'all', 'loss']:
    ...
    def __call__(self, z_t):
        ...
        x_cycles, y_cycles = self.ncycles(z_t)  # (bs, n_nodes, 3)  (bs, 4)
        ...
        if self.features_type == 'cycles':
            ...
            eigenfeatures = self.eigenfeatures(z_t)  # (bs, 1), (bs, k=5)
            E = z_t.E
            extra_edge_attr = torch.zeros((*E.shape[:-1], 0), device=E.device)
            ...
            return utils.PlaceHolder(X=torch.cat((x_cycles, nonlcc_indicator, k_lowest_eigvec), dim=-1),
                                     E=extra_edge_attr,
                                     y=torch.hstack((n, y_cycles, n_components, batched_eigenvalues)),
                                     pos=None, charges=None)
        elif self.features_type == 'loss':
            n_components, _, _, _ = self.eigenfeatures(z_t)
            return n_components, y_cycles
...
class EigenFeatures:
    ...
    def __call__(self, z_t):
        ...
        elif self.mode == 'all' or self.mode == 'loss':
...
class NodeCycleFeaturesSingle:
    ...
    def __call__(self, adj_matrix):
        ...
        _, y_cycles = self.kcycles.k_cycles(adj_matrix=adj_matrix.to(torch.float))  # (bs, n_cycles)
        y_cycles = y_cycles / 10
        y_cycles[y_cycles > 1] = 1
        return y_cycles


class EigenFeatureSingleGraph(EigenFeatures):
    def __init__(self):
        super(EigenFeatureSingleGraph, self).__init__(mode='all')
        self.ncycles = NodeCycleFeaturesSingle()

    def __call__(self, graph_data):
        A = torch_geometric.utils.to_dense_adj(edge_index=graph_data.edge_index, edge_attr=graph_data.edge_attr,
                                               max_num_nodes=graph_data.x.size(0))
        L = compute_laplacian(A, normalize=False)
        mask = torch.ones(A.size(0), graph_data.x.size(0), dtype=torch.bool)  # bs, n
        ...
        if self.mode == 'eigenvalues':
            ...
            y_cycles = self.ncycles(A)
            eigvals, eigvectors = torch.linalg.eigh(L)
            eigvals = eigvals / torch.sum(mask, dim=1, keepdim=True)
            n_connected_comp, batch_eigenvalues = get_eigenvalues_features(eigenvalues=eigvals)
            return n_connected_comp, y_cycles
...
class KNodeCycles:
    ...
    def calculate_kpowers(self):
        self.k1_matrix = self.adj_matrix
    ...
    def k_cycles(self, adj_matrix, verbose=False):
        self.adj_matrix = adj_matrix.to(torch.float)
