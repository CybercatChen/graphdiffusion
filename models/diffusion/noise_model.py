...
class NoiseModel:
    ...
    def move_P_device(self, tensor):
        """将存储在类中的转移矩阵（例如 Px, Pe, Py）转换到与输入张量相同的设备
    ...
    def get_Qt(self, t_int):
        """ 计算从第 t-1 步到 t 步的一步转移矩阵
            t_int 表示当前噪声步数
        Returns one-step transition matrices for X and E, from step t - 1 to step t.
        Qt = (1 - beta_t) * I + beta_t / K
        beta_t: (bs)                         noise level between 0 and 1
        returns: qx (bs, dx, dx), qe (bs, de, de), qy (bs, dy, dy).
        返回一个 PlaceHolder 对象，其中包含各个特征的转移矩阵
        """
    ...
    def apply_noise(self, dense_data):
        ...
        # 计算累计转移矩阵，然后根据这些矩阵计算各个离散特征的转移概率
    ...
    def sample_zs_from_zt_and_pred(self, z_t, pred, s_int, is_directed=False):
        """ 在采样（逆扩散）过程中，根据当前时刻的噪声数据 z_t 和模型预测的去噪结果 pred，从后验分布中采样得到前一步的隐变量 z_s
...
class MarginalUniformTransition(NoiseModel):
    """ 利用传入的边缘分布（marginals）来构造转移矩阵
        与前一个子类不同，这里允许用户指定不同特征的边缘分布（例如节点类型和边的边缘分布），从而使得噪声过程更符合数据的统计特性"""
