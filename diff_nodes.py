...
class Simple_FC(nn.Module):
    ...
    def pos_encoding(self, t, proj_dims):
        inv_freq = 1.0 / (10000 ** (torch.arange(0, proj_dims, 2, device=t.device).float() / proj_dims))
    ...
    def forward(self, pos, t, node_mask):
        """
        Conditioning made on the latents and their original position
...
def perform_coord_wise_kde(coords_synth, coords_real, epoch):
    ...
    os.makedirs('./stage1/node_diffusion', exist_ok=True)
    plt.savefig(f'./stage1/node_diffusion/line_graph_attr_{epoch}.png', dpi=80)
    plt.show()
...
def get_data(args):
    train_dataset = CoWDataset(dataset_name=args.dataset, split='train', data_path=args.datapath, config=args)
    test_dataset = CoWDataset(dataset_name=args.dataset, split="test", data_path=args.datapath, config=args)
...
def train(args):
    ...
    mse = nn.MSELoss()
    logger = SummaryWriter(os.path.join("stage1/runs", args.run_name))
        ...
        if epoch % 1000 == 0:
            with torch.no_grad():
                ...
                if icp_loss < best_val_loss:
                    torch.save(model.state_dict(), os.path.join(PROJECT_ROOT_DIR, 'models',
...
def test(args):
    ...
    model.load_state_dict(torch.load(os.path.join(PROJECT_ROOT_DIR, 'models',
                                                  f'checks_{args.dataset}_best_coord_diff_model.pth')))
    model.to(device)
    sampled_positions, actual_positions = [], []
    model.eval()
...
def launch():
    ...
    # args.dataset = 'cow'
    args.dataset = 'march'
    args.datapath = r'D:\PartVessel\vessel_dif\data\march'
    args.lr = 3 * 1e-4
